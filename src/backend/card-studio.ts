...
const GENERATION_REQUIREMENTS: CardStudioGenerationRequirements = {
  ...
  referencePolicy: [
    ...
    "Обязательный путь генерации: открыть авторизованный ChatGPT через Browser, приложить исходное фото товара в composer как файл/clipboard-изображение и отправлять промпты уже с этим attachment/reference в чате.",
  ...
  generationToolPolicy: [
    "Для финальных слайдов агент использует Browser-контур с авторизованным ChatGPT: вставить/прикрепить исходное фото, отправить промпт, дождаться генерации, скачать/забрать PNG из ответа браузера, проверить и только потом загрузить в MPFlow.",
...
Парадигма: исходное фото — главный контракт. Качество исходника не важно — важны ДЕТАЛИ, которые на нём видны. Рабочий контур для финальных слайдов — Browser с авторизованным ChatGPT, где исходник прикреплён в composer как реальный файл/изображение. Точный промпт + обязательный attachment/reference → хороший результат. Но товар на выходе обязан быть идентичен референсу.

## Жёсткие правила
- Референс обязателен для каждого финального слайда. Используй Browser-контур с авторизованным ChatGPT: открой chatgpt.com, прикрепи исходное фото товара в чат, отправляй промпты вместе с этим attachment/reference и забирай готовые PNG из ответа.
...
- Для финальной генерации используй Browser с авторизованным ChatGPT: вставь исходное фото в composer как attachment/reference (файлом или через clipboard), затем отправляй промпт. Не полагайся только на view_image в Codex-контексте.
...
- Browser-контур: открой авторизованный ChatGPT, прикрепи исходное фото товара, отправь промпт на один или несколько слайдов, дождись генерации, скачай/забери PNG из ответа браузера, сохрани локальную копию, проверь визуально совпадение товара и тексты.
