...
export interface CardStudioGenerationRequirements {
  ...
  browserChatGptRequired: boolean;
...
const GENERATION_REQUIREMENTS: CardStudioGenerationRequirements = {
  ...
  browserChatGptRequired: true,
  competitorResearchRequired: true,
  factGroundingRequired: true,
  visualDiversityRequired: true,
  referencePolicy: [
    "Перед любой генерацией изображения агент обязан использовать исходное фото товара как visual reference/edit target. Текстового описания товара недостаточно.",
    "Обязательный путь всей генерации: открыть авторизованный ChatGPT через @Браузер/Browser, приложить исходное фото товара в composer как файл/clipboard-изображение и отправлять промпты уже с этим attachment/reference в чате.",
  ...
  generationToolPolicy: [
    "Всю генерацию изображений агент выполняет через @Браузер/Browser-контур с авторизованным ChatGPT: вставить/прикрепить исходное фото, отправить промпт, дождаться генерации, скачать/забрать PNG из ответа браузера, проверить и только потом загрузить в MPFlow.",
    ...
    "Если текст или товар заметно искажены, такой слайд не загружать; повторить промпт короче и строже в том же Browser-контуре.",
    "Локальная программная сборка/композитинг не является генерационным контуром фотостудии и не должна загружаться как финальный generated-слайд."
  ],
  researchPolicy: [
...
Парадигма: исходное фото — главный контракт. Качество исходника не важно — важны ДЕТАЛИ, которые на нём видны. Рабочий контур для всей генерации изображений — @Браузер/Browser с авторизованным ChatGPT, где исходник прикреплён в composer как реальный файл/изображение. Точный промпт + обязательный attachment/reference → хороший результат. Но товар на выходе обязан быть идентичен референсу.

## Жёсткие правила
- Референс обязателен для каждой генерации изображения. Используй @Браузер/Browser-контур с авторизованным ChatGPT: открой chatgpt.com, прикрепи исходное фото товара в чат, отправляй промпты вместе с этим attachment/reference и забирай готовые PNG из ответа.
...
- Для генерации используй @Браузер/Browser с авторизованным ChatGPT: вставь исходное фото в composer как attachment/reference (файлом или через clipboard), затем отправляй промпт. Не полагайся только на view_image в Codex-контексте.
...
- @Браузер/Browser-контур: открой авторизованный ChatGPT, прикрепи исходное фото товара, отправь промпт на один или несколько слайдов, дождись генерации, скачай/забери PNG из ответа браузера, сохрани локальную копию, проверь визуально совпадение товара и тексты.
