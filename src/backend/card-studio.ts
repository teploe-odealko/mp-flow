...
const GENERATION_REQUIREMENTS: CardStudioGenerationRequirements = {
  ...
  referencePolicy: [
    "Перед генерацией каждого финального слайда агент обязан использовать исходное фото товара как visual reference/edit target, если инструмент генерации это поддерживает.",
    "Если инструмент не умеет надежно сохранять товар по референсу, финальный слайд нужно собирать гибридно: фон/сцену генерировать отдельно, а реальный товар накладывать из исходника.",
  ...
  factPolicy: [
    ...
    "Каждое утверждение на слайде должно опираться на один из источников: видимые детали исходного фото, данные товара в брифе, привязанная карточка/поля MPFlow или прямое указание пользователя.",
    "Если преимущество выглядит вероятным по категории, но не подтверждено для товара, его можно использовать только как вопрос в плане или не использовать в тексте слайда."
  ],
  planFields: [
...
## Жёсткие правила
- Референс обязателен для каждого финального слайда. Если инструмент генерации поддерживает image reference / edit target — передавай исходное фото в каждый запрос. Если инструмент плохо сохраняет товар, генерируй фон/сцену отдельно и собирай финальный слайд с реальным товаром из исходника.
...
- Ничего не придумывай. Любой текст на слайде должен опираться на факты из брифа, видимые детали исходного фото, поля товара/карточки MPFlow или прямые слова пользователя.
...
- Если факт не подтверждён — занеси его в unknowns/claimsToAvoid и не используй в финальной картинке.
