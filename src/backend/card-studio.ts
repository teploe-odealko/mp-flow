...
const GENERATION_REQUIREMENTS: CardStudioGenerationRequirements = {
  ...
  browserChatGptPreferred: true,
  competitorResearchRequired: true,
  factGroundingRequired: true,
  visualDiversityRequired: true,
  referencePolicy: [
    "Перед генерацией каждого финального слайда агент обязан использовать исходное фото товара как visual reference/edit target. Текстового описания товара недостаточно.",
    "Предпочтительный путь: открыть авторизованный ChatGPT через Browser, приложить исходное фото товара в composer как файл/clipboard-изображение и отправлять промпты уже с этим attachment/reference в чате.",
    "Встроенный image_gen Codex допускается только как fallback/черновик, если Browser/ChatGPT недоступен или пользователь явно попросил image_gen; для финальных слайдов он хуже удерживает точную геометрию товара.",
  ...
  generationToolPolicy: [
    "Для финальных слайдов агент сначала пробует Browser-контур с авторизованным ChatGPT: вставить/прикрепить исходное фото, отправить промпт, дождаться генерации, скачать/забрать PNG из ответа браузера, проверить и только потом загрузить в MPFlow.",
    "Если ChatGPT сгенерировал несколько картинок не в том порядке, агент определяет slideType по фактическому содержанию изображения, а не по порядку выдачи.",
    "Если текст или товар заметно искажены, такой слайд не загружать; повторить промпт короче и строже или оставить результат как черновик.",
    "Локальная программная сборка/композитинг допустимы только для проверки гипотез или когда нужно сохранить реальные пиксели товара; не выдавать такой результат за LLM-generated без пометки."
  ],
  researchPolicy: [
...
Парадигма: исходное фото — главный контракт. Качество исходника не важно — важны ДЕТАЛИ, которые на нём видны. Лучший рабочий контур для финальных слайдов — Browser с авторизованным ChatGPT, где исходник прикреплён в composer как реальный файл/изображение. Точный промпт + обязательный attachment/reference → хороший результат. Но товар на выходе обязан быть идентичен референсу.

## Жёсткие правила
- Референс обязателен для каждого финального слайда. Предпочтительно используй Browser-контур с авторизованным ChatGPT: открой chatgpt.com, прикрепи исходное фото товара в чат, отправляй промпты вместе с этим attachment/reference и забирай готовые PNG из ответа.
- Встроенный image_gen Codex используй только как fallback/черновик, если Browser/ChatGPT недоступен или пользователь явно попросил именно image_gen. Для финальных карточек он может перерисовать геометрию товара, даже если исходник виден через view_image.
...
- Если доступен Browser с авторизованным ChatGPT, используй именно его для финальной генерации: вставь исходное фото в composer как attachment/reference (файлом или через clipboard), затем отправляй промпт. Не полагайся только на view_image в Codex-контексте.
...
- Рекомендуемый Browser-контур: открой авторизованный ChatGPT, прикрепи исходное фото товара, отправь промпт на один или несколько слайдов, дождись генерации, скачай/забери PNG из ответа браузера, сохрани локальную копию, проверь визуально совпадение товара и тексты.
- Если ChatGPT вернул картинки не в том порядке, определяй slideType по фактическому содержанию: hero/benefits/lifestyle/etc. Не загружай слайд под неверным типом только из-за порядка ответа.
- Каждый готовый слайд: card_studio_create_upload (role=generated, slideType из плана или по фактическому содержанию, contentType) → HTTP PUT байтов на выданный uploadUrl → card_studio_confirm_asset.
