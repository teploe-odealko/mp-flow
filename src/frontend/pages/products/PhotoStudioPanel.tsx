        ...
        <CardContent className="py-5 flex flex-col gap-4">
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div>
              ...
              <div className="max-w-xl text-sm text-[var(--color-muted-foreground)]">
  ...
  return (
    <Card className="renderPanel">
      <CardContent className="py-5 flex flex-col gap-3">
        ...
        {research && <div className="line-clamp-3 text-sm text-[var(--color-muted-foreground)]">{research}</div>}
        {slides.length > 0 && (
          <ol className="flex flex-col gap-1.5">
            {slides.map((slide, index) => (
              <li key={index} className="flex gap-2 text-sm">
                <span className="w-5 shrink-0 text-[var(--color-muted-foreground)]">{index + 1}.</span>
                <span className="shrink-0 font-medium">{slide?.type ?? slide?.title ?? "слайд"}</span>
                {(slide?.idea ?? slide?.message ?? slide?.purpose) && (
                  <span className="truncate text-[var(--color-muted-foreground)]">— {slide.idea ?? slide.message ?? slide.purpose}</span>
                )}
              </li>
            ))}
          </ol>
  ...
  return (
    <Card className="renderPanel">
      <CardContent className="py-5">
        ...
        <div className="mb-4 text-sm text-[var(--color-muted-foreground)]">{hint}</div>
        {assets.length === 0 ? (
          <EmptyState icon={<ImagePlus size={20} />} title={emptyText ?? "Пусто"} />
        ) : (
          <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5">
            {assets.map((asset) => (
              <div key={asset.id} className="group relative overflow-hidden rounded-[var(--radius-lg)] border border-[var(--color-border)] bg-[var(--color-muted,#f4f4f5)]">
                <div className="flex aspect-[3/4] w-full items-center justify-center overflow-hidden">
                ...
                <div className="flex items-center justify-between gap-1 px-2 py-1.5">
                  <div className="flex min-w-0 items-center gap-1">
                    ...
                    {asset.slideType && <span className="truncate text-[11px] text-[var(--color-muted-foreground)]">{asset.slideType}</span>}
