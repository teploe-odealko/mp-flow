...
const MASTER_CARD_MODULE = "masterCardModuleService"
const FIFO_LOT_MODULE = "fifoLotModuleService"
const FINANCE_MODULE = "financeModuleService"
...
async function unitEconomicsReport(
  ...
  const rows = Object.values(byOffer).map((entry: any) => {
    const totalFees =
      entry.commission + entry.last_mile + entry.pipeline +
      entry.fulfillment + entry.direct_flow_trans + entry.reverse_flow_trans +
      entry.return_processing + entry.acquiring + entry.marketplace_service +
      entry.other_fees
...
async function pnlReport(
  ...
  const financeService: any = req.scope.resolve(FINANCE_MODULE)
  const ozonService: any = req.scope.resolve(OZON_MODULE)
  ...
  const totalOzonFees =
    totalCommission + totalLastMile + totalPipeline +
    totalFulfillment + totalDirectFlow + totalReverseFlow +
    totalReturnProcessing + totalAcquiring + totalMarketplace +
    totalOtherFees
