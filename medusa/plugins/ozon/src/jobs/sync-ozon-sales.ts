...
export default async function handler(container: MedusaContainer) {
  ...
  for (const account of accounts) {
    try {
      const { syncOzonSalesWorkflow } = await import(
        "../workflows/sync-ozon-sales.js"
      )
      await syncOzonSalesWorkflow(container).run({
        input: { account_id: account.id },
      })
    } catch (err: any) {
      console.error(`[sync-ozon-sales] Failed for account ${account.name}:`, err.message)
