...
export class Migration20260301145518 extends Migration {
  ...
  override async up(): Promise<void> {
    // Drop old tables to recreate with new schema (added type, user_id columns)
    ...
    // Also drop fifo-lot tables that no longer exist as a module
    this.addSql(`drop table if exists "fifo_allocation" cascade;`);
    this.addSql(`drop table if exists "fifo_lot" cascade;`);
    this.addSql(`create table "supplier" ("id" text not null, "user_id" text null, "name" text not null, "contact" text null, "phone" text null, "notes" text null, "metadata" jsonb null, "created_at" timestamptz not null default now(), "updated_at" timestamptz not null default now(), "deleted_at" timestamptz null, constraint "supplier_pkey" primary key ("id"));`);
