...
Откройте `.env` и задайте секреты:

```bash
# Обязательные (измените перед использованием)
MEDUSA_DB_PASSWORD=ваш_пароль_для_бд
MEDUSA_COOKIE_SECRET=сгенерируйте_openssl_rand_-base64_32
MEDUSA_JWT_SECRET=сгенерируйте_openssl_rand_-base64_32
```
...
| `COOKIE_SECRET` | Прод | Секрет для подписи session cookies. Без него cookies можно подделать. Для локальной разработки не нужен. Генерируйте: `openssl rand -base64 32` |
| `JWT_SECRET` | Прод | Секрет для подписи JWT auth-токенов. Для локальной разработки не нужен. Генерируйте: `openssl rand -base64 32` |
