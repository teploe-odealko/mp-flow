...
│   - entities, services, routes │
│   - middleware, nav, columnDocs │
└──────┬──────────────────────────┘
       │
┌──────▼──────┐
│ PostgreSQL  │
...
Плагины подключаются в `mpflow.config.ts`. Таблицы для плагинов создаются автоматически при старте. Подробнее — [Разработка плагинов](/docs/developer/plugin-development).
