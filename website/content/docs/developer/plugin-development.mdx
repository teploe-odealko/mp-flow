...
```bash
# Собрать плагин (admin-расширения + серверный код)
cd medusa/plugins/my-plugin
...
`plugin:build` компилирует admin-расширения (React). `tsc` компилирует серверный код (модули, API-роуты) в `.medusa/server/`, откуда Medusa их загружает.
...
Команда `db:generate` принимает **имя модуля** (значение константы из `Module()`), а не имя npm-пакета:

```bash
cd medusa/
npx medusa db:generate myModuleService
```

Миграция создастся в `node_modules/mpflow-plugin-my-plugin/.medusa/server/src/modules/my-module/migrations/`. Скопируйте её обратно в исходники плагина:

```bash
# Скопировать миграции из node_modules обратно в исходники
cp -R node_modules/mpflow-plugin-my-plugin/.medusa/server/src/modules/my-module/migrations/ \
  plugins/my-plugin/src/modules/my-module/migrations/
```

Пересоберите плагин с миграциями и примените:

```bash
# Пересобрать плагин (теперь с миграциями в исходниках)
cd plugins/my-plugin
npx medusa plugin:build
npx tsc --project tsconfig.json
cd ../..

# Обновить в node_modules
rm -rf node_modules/mpflow-plugin-my-plugin/.medusa
cp -R plugins/my-plugin/.medusa node_modules/mpflow-plugin-my-plugin/

# Скопировать snapshot-файлы (tsc не копирует .json из migrations)
cp plugins/my-plugin/src/modules/my-module/migrations/.snapshot-*.json \
  node_modules/mpflow-plugin-my-plugin/.medusa/server/src/modules/my-module/migrations/ 2>/dev/null || true
