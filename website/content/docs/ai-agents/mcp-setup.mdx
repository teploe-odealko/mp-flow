...
description: Настройка MCP подключения к MPFlow для Claude Desktop, ChatGPT, Cursor и других AI-клиентов
---

MPFlow предоставляет **54 MCP инструмента** через [Model Context Protocol](https://modelcontextprotocol.io/). Любой MCP-совместимый клиент может подключиться.
...
MPFlow использует **Streamable HTTP** транспорт — стандартный MCP транспорт для веб-серверов. Endpoint: `/mcp`.
