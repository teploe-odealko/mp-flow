...
def parse_ozon_postings(payload: dict[str, Any]) -> list[dict[str, Any]]:
    """
    Parse Ozon posting list response into normalized sale structures.
    """
    result = payload.get("result") if isinstance(payload, dict) else None
    postings = []
    if isinstance(result, dict):
        ...
        postings = result
    if not postings and isinstance(payload, dict):
        postings = payload.get("postings") or payload.get("items") or []
    ...
    for posting in postings:
        ...
        external_order_id = str(
            ...
            or posting.get("id")
            or _safe_external_id("ozon_posting", posting)
        )
        sold_at = (
            posting.get("in_process_at")
            or posting.get("shipment_date")
            or posting.get("created_at")
            or posting.get("order_date")
            or datetime.utcnow().isoformat()
        )

        fee_by_key: dict[str, Decimal] = {}
        ...
        if isinstance(financial_data, dict):
            ...
            if isinstance(financial_products, list):
                for fin_product in financial_products:
                    if not isinstance(fin_product, dict):
                        continue
                    fee_raw = to_money(
                        fin_product.get("commission_amount")
                        or fin_product.get("sale_commission")
                        or fin_product.get("delivery_amount")
                        or 0
                    )
                    fee_value = to_money(abs(fee_raw))
                    if fee_value <= 0:
                        continue
                    for key in (
                        fin_product.get("offer_id"),
                        fin_product.get("product_id"),
                        fin_product.get("sku"),
                        fin_product.get("id"),
                    ):
                        if key in (None, ""):
                            continue
                        fee_by_key[str(key)] = fee_value
        ...
        for product in products:
            if not isinstance(product, dict):
                continue
            offer_id = str(
                product.get("offer_id") or product.get("sku") or product.get("article") or ""
            )
            if not offer_id:
                continue

            quantity = to_qty(product.get("quantity") or product.get("quantity_deliver") or 0)
            if quantity <= 0:
                continue

            unit_price = to_money(
                product.get("price")
                or product.get("price_with_discount")
                or product.get("price_with_disc")
                or product.get("sale_price")
                or 0
            )
            fee_raw = to_money(
                product.get("commission_amount")
                or product.get("sale_commission")
                or product.get("delivery_amount")
                or 0
            )
            fee = to_money(abs(fee_raw))
            if fee <= 0:
                fee = (
                    fee_by_key.get(str(product.get("offer_id") or ""))
                    or fee_by_key.get(str(product.get("sku") or ""))
                    or fee_by_key.get(str(product.get("product_id") or ""))
                    or Decimal("0.00")
                )
            items.append(
                {
