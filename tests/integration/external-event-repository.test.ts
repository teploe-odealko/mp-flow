...
describePostgres("ExternalEventRepository", () => {
  it("читает поток событий из таблицы, минуя snapshot", async () => {
    ...
    const session = await store.openWriteSession();
    ...
    try {
      ...
      expect(await repo.count({ channelId })).toBe(2);

      const byChannel = await repo.list({ channelId });
      ...
      const sales = await repo.list({ channelId, eventType: "sale" });
      expect(sales).toHaveLength(2);

      const paged = await repo.list({ channelId, limit: 1, offset: 0 });
      expect(paged).toHaveLength(1);

      const found = await repo.findByExternalId(channelId, "S1");
      expect(found?.externalId).toBe("S1");

      const byId = await repo.getById(byChannel[0].id);
      expect(byId?.id).toBe(byChannel[0].id);

      expect(await repo.findByExternalId(channelId, "NOPE")).toBeUndefined();
    } finally {
      await pool.end();
      await store.close();
    }
