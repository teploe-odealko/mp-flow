...
// ── Helpers ──
async function countUsers(c) {
    const orm = c.get("orm");
    const em = orm.em.fork();
    const conn = em.getConnection();
    const result = await conn.execute(`SELECT COUNT(*)::int AS cnt FROM mpflow_user WHERE deleted_at IS NULL`);
...
async function findUserByEmail(c, email) {
    ...
    const em = orm.em.fork();
    const conn = em.getConnection();
    const result = await conn.execute(`SELECT id, email, name, password_hash FROM mpflow_user WHERE email = ? AND deleted_at IS NULL LIMIT 1`, [email]);
...
auth.post("/setup", async (c) => {
    ...
    await conn.execute(`INSERT INTO mpflow_user (id, email, name, password_hash, created_at, updated_at)
     VALUES (?, ?, ?, ?, NOW(), NOW())`, [userId, email, name || null, passwordHash]);
    // Set session
