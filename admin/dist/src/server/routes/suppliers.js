...
suppliers.get("/", async (c) => {
    const service = c.get("container").resolve("supplierOrderService");
...
suppliers.put("/:id", async (c) => {
    ...
    if (body.items) {
        ...
        for (const existing of existingItems)
            await service.deleteSupplierOrderItems(existing.id);
        for (const item of body.items) {
            const purchaseRub = Number(item.purchase_price_rub || 0);
            const packaging = Number(item.packaging_cost_rub || 0);
            const logistics = Number(item.logistics_cost_rub || 0);
...
suppliers.delete("/:id", async (c) => {
    ...
    if (userId && order.user_id && order.user_id !== userId)
        return c.json({ error: "Not found" }, 404);
    if (order.status !== "draft")
        return c.json({ error: "Can only delete draft orders" }, 400);
...
suppliers.post("/:id", async (c) => {
    ...
    if (body.action === "receive") {
        if (!body.items?.length)
            return c.json({ error: "Items with received quantities required" }, 400);
