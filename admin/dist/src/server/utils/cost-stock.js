 ...
 */
export async function calculateAvgCost(supplierOrderService, masterCardId) {
 ...
 */
export async function getAvailableStock(supplierOrderService, saleService, masterCardId) {
    const items = await supplierOrderService.listSupplierOrderItems({
        master_card_id: masterCardId,
        received_qty: { $gt: 0 },
    });
    ...
    const totalSold = sales.reduce((s, sale) => s + Number(sale.quantity || 0), 0);
    return totalReceived - totalSold;
}
//# sourceMappingURL=cost-stock.js.map