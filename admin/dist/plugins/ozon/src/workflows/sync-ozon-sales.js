...
function mapOzonStatus(ozonStatus) {
    const map = {
...
export async function syncOzonSales(container, accountId, dateFrom, dateTo) {
    ...
    for (const posting of postings) {
        ...
        for (const product of postingProducts) {
            ...
            try {
                ...
                if (links.length > 0)
                    masterCardId = links[0].master_card_id || null;
            }
            catch { /* skip */ }
            // Calculate avg cost
            let unitCogs = 0;
